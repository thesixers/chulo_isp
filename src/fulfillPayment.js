...
export async function fulfillPayment(db, sock, user, amountPaid) {
  ...
  if (giftTargetUserId) {
    const targetRes = await db.query(`SELECT * FROM users WHERE id = $1`, [
      giftTargetUserId,
    ]);
    ...
    if (isRenewal) {
      ...
      await sock.sendMessage(remoteJid, {
        text:
          ...
          (isGift
            ? `🎁 Gifted to: *${targetUser.hotspot_username}*\n\n`
            : `\n`) +
          `⏳ *Plan Queued*\n` +
...
export async function provisionOrQueue(
  ...
  try {
    const comment = expiryTime
      ? buildMikrotikComment(user.phone, plan.duration_days, expiryTime)
      : null;
    ...
    console.error(
      "MikroTik provisioning failed — queuing for retry:",
      err.message,
    );
