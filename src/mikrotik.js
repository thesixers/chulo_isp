...
export async function removeHotspotUser(username) {
  ...
  try {
    ...
    console.warn(
      `⚠️  MikroTik: user '${username}' not found — already removed`,
    );
  } finally {
    conn.close();
  }
...
/**
 * Removes all active hotspot sessions for a given username from MikroTik.
 * This is needed when a user's subscription is renewed or they change their password,
 * to ensure the old sessions are terminated and they must log in again with the new details.
 * @param {string} username - The hotspot username whose sessions should be removed
 */
export async function removeActiveSessions(username) {
  ...
  try {
    ...
    if (!sessions.length) {
      console.log(`ℹ️  MikroTik: no active session for '${username}' — skipping`);
      return 0;
    }
    ...
    console.log(`🗑️  MikroTik: removed ${sessions.length} active sessions for '${username}'`);
    return sessions.length;
  } catch (err) {
    console.warn(`⚠️  MikroTik: removeActiveSessions failed for '${username}':`, err.message);
  } finally {
    conn.close();
  }
};
